...
def main(cfg):
    ...
    ply_file = '/home/haozhe/Dropbox/rendering/asset/final_scene_with_ids.ply'
    ...
    recenter_list[15]= np.array([-0.943, -0.179, 0.185])
    # 3. Loop over semantic IDs and apply transformations

    # inv_joint_value=torch.from_numpy(inv_joint_value).type(torch.float)

    # value one
    # joint_control_action= torch.tensor([-0.15, -0.435 ,0.07, -1.42, 1.726 ,1.048, 0.506,0,0,0,0,0], dtype=torch.float)

    # value two
    # robot_action = np.array([-0.47, 0.07, 0.07, -1.6, 1.5, 1.186, 0.695])
    inv_joint_value = torch.tensor([-0.47, 0.07, 0.07, -1.53, 1.5, 1.186, 0.695,
                                    0,
                                    0,0,0,0], dtype=torch.float)
                                    
    # joint_control_action= torch.tensor([-0.43, -0.105 ,0.07, -1.65, 1.476 ,1.048, 0.626,
    #                                     0,
    #                                     0,0,0,0], dtype=torch.float)

    # grasp pose
    # joint_control_action = torch.tensor([-1, -1, -1, -0.6, 1.5, 1.186, 0.695,0,0,0,0,0])
    joint_control_action= torch.tensor([-0.3, -0.205 ,0.07, -1.72, 1.426 ,0.848, 0.4,0,0,0,0,0], dtype=torch.float)
    # for index in range(len(active_id)):
    #     # Extract the inverse and forward transforms
    #     step= 250
    ...
    #     # Shift points by the center (if needed)
    ...
    save_ply_sam(
        ...
        path='/home/haozhe/Dropbox/rendering/asset/arm_deform_20.ply',
...
def renderer(cfg,parser,input_ply='/home/haozhe/Dropbox/rendering/demo/gs_result_sfm/scan30000.ply',device='cuda'):          
    ...
    for i in tqdm.trange(10, desc="Rendering trajectory"):
        camtoworlds = camtoworlds_all[i : i + 1]
        Ks = K[None]
        xyz, features_dc, features_extra, opacities, scales, rots = load_ply(input_ply)


        splats["means"] = torch.from_numpy(xyz).to(device)  # [N, 3]
        splats["quats"] = torch.from_numpy(rots).to(device)  # [N, 4]
        splats["scales"]   = torch.from_numpy(scales).to(device)  # [N, 3]
        splats["opacities"]    = torch.from_numpy(opacities).to(device).view(scales.shape[0])  # [N,]
        splats['sh0']  = torch.from_numpy(features_dc).view(features_dc.shape[0],features_dc.shape[2],features_dc.shape[1]).to(device)  # [N, K, 3]
        splats['shN']  = torch.from_numpy(features_extra).view(features_extra.shape[0],features_extra.shape[2],features_extra.shape[1]).to(device)  # [N, K, 3]
